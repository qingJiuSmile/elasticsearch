package com.weds.demo.elasticsearch.util;

import com.alibaba.fastjson.JSON;
import com.weds.demo.elasticsearch.entity.es.EsData;
...
public class EsClientUtil {
     ...
     * 自定义批量添加（主要用于自定义id）
     *
     * @param request 拼装好的请求
     * @return boolean
     * @author tjy
     * @date 2020/7/15
     **/
    public boolean bulkAddDocument(BulkRequest request) throws IOException {
        BulkResponse bulkResponse = client.bulk(request, RequestOptions.DEFAULT);
        // 是否失败返回false 代表成功
        return !bulkResponse.hasFailures();
    }
     ...
     * 批量修改
     *
     * @param request   批量修改的请求体
     * @return boolean
     * @author tjy
     * @date 2020/7/13
     **/
    public boolean bulkUpdateDocument(BulkRequest request) throws IOException {
        BulkResponse bulkResponse = client.bulk(request, RequestOptions.DEFAULT);
        // 是否失败返回false 代表成功
        return !bulkResponse.hasFailures();
     ...
     * @date 2020/7/14
     **/
    public EsReturnData matchAllQuery(Integer form, Integer size, String sort,
                                      String... indexName) throws IOException {
        SearchRequest searchRequest = new SearchRequest(indexName);
        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
    ...
    public EsReturnData matchOperatorQuery(Integer form, Integer size, String sort, String docName,
                                           Operator operator, List<String> value, String... indexName) throws IOException {

        SearchRequest searchRequest = new SearchRequest(indexName);
        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
        searchSourceBuilder.query(QueryBuilders.matchQuery(docName, listToString(value, " ")).operator(operator));
        /*
         ******** 分页 ********
         */
        if (form != null && form > 0) {
     ...
     * @date 2020/7/15
     **/
    public boolean addIndexTemplate(String templateName, String configName,
                                    List<String> patterns, String alias,Integer order) throws IOException {

        PutIndexTemplateRequest request = new PutIndexTemplateRequest(templateName);

        // 从配置文件中规定配置信息
        if (configName != null && !"".equals(configName)) {
            String path = "classpath:" + configName + ".json";
     ...
     * 删除索引模板
     *
     * @param indexTmpName 模板名称
     * @return boolean
     * @author tjy
     * @date 2020/7/15
     **/
    public boolean delIndexTemplate(String indexTmpName) throws IOException {
        DeleteIndexTemplateRequest request = new DeleteIndexTemplateRequest(indexTmpName);
    ...
    /**
     * 判断是否有对应的模板
     * @param tmpList   模板名称集合
     * @author tjy
     * @date 2020/7/15
     * @return boolean
     **/
    public boolean existsIndexTemplate(List<String> tmpList) throws IOException {
        IndexTemplatesExistRequest request;
        // 按照模板名称
        request = new IndexTemplatesExistRequest(tmpList);
